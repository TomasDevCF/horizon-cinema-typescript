  ...
  return (
    <header className="w-100 py-3 px-2 d-flex flex-row align-items-center ">
            ...
            <p onClick={() => setCategoriesActived(!categoriesActived)} className="m-0 fs-4 d-flex align-items-center text-white gap-2 text-decoration-none header-link position-relative">
              ...
              {categoriesActived && <div className="categories position-absolute rounded d-flex flex-column align-items-center px-2 py-2">
                {categories && categories.genres.map(genre => {
                  return <div className="category rounded text-center my-1 py-1"><Link to={`/category/${genre.id}/1`} className="h7 m-0 text-decoration-none text-white">{categoryTranslator[genre.id as keyof ICategoryTranslate]}</Link></div>
                })}
              </div>}
            </p>
      ...
      <section className="left-section col-lg-4 d-none d-lg-block">
        <Link to="/" className="h3 text-decoration-none px-2" style={{ color: "rgb(42, 151, 253)" }}>Horizon Cinema</Link>
      </section>
      <section className="center-section col-lg-4 position-relative d-none align-items-center justify-content-around d-lg-flex">
        <Link onClick={savedLinkClick} className="text-white text-decoration-none header-link" to="/saved/1"><span className="fa-regular fa-bookmark"></span> Guardados</Link>
        <Link className="text-white text-decoration-none header-link" to="/movies"><span className="fa-sharp fa-solid fa-film"></span> Cartelera</Link>
        <p onClick={() => setCategoriesActived(!categoriesActived)} className="m-0 text-white text-decoration-none header-link"><span className={`fa-solid fa-caret-${categoriesActived ? "up" : "down"}`}></span> Categorías</p>
        {categoriesActived && <div className="categories position-absolute rounded d-flex flex-column align-items-center px-2 py-2">
          {categories && categories.genres.map(genre => {
            return <div className="category rounded text-center my-1 py-1"><Link to={`/category/${genre.id}/1`} className="h7 m-0 text-decoration-none text-white">{categoryTranslator[genre.id as keyof ICategoryTranslate]}</Link></div>
          })}
